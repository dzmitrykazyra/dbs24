...
where uc.user_id = ac.user_id
     ...
     c_DeviceByPhoneCode as (select distinct phone_code, device_type_id
                             from c_ContractsByPhoneCodeAndDeviceTypeId)
select CASE device_type_id WHEN 0 THEN 'android' WHEN 1 THEN 'ios' END device,
       dfc.phone_code,
       (select count(*)
        from c_ActualContractsByPhoneCodeAndDeviceTypeId asb
        where asb.device_type_id = dfc.device_type_id
...
order by dfc.device_type_id desc, dfc.phone_code asc