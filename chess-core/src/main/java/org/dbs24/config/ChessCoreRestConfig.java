...
public class ChessCoreRestConfig extends AbstractWebSecurityConfig {

    @Bean
    public RouterFunction<ServerResponse> restChessBean(ChessRestApi chessRestApiHandler) {

        return addCommonRoutes()
                .andRoute(POST(URI_CREATE_CHESS_PLAYER).and(accept(MediaType.APPLICATION_JSON)), chessRestApiHandler::createChessPlayer)
