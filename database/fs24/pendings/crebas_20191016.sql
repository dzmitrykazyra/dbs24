...
/* Created on:     16.10.2019 18:02:10                          */
/*==============================================================*/
...
create table TariffCalculations (
   ...
   constraint PK_TARIFFCALCULATIONS primary key (calculate_date, tariff_serv_id, tariff_kind_id, contract_id)
);
...
alter table TariffCalculations
   add constraint FK_TARIFFCA_TC_CONTRA_CORE_ENT foreign key (contract_id)
      references core_EntityContracts (contract_id)
      on delete restrict on update restrict;

alter table TariffCalculations
   add constraint FK_TARIFFCA_TC_CURREN_CORE_CUR foreign key (currency_id)
      references core_CurrenciesRef (currency_id)
      on delete restrict on update restrict;

alter table TariffCalculations
   add constraint FK_TARIFFCA_TC_TARIFF_TARIFFKI foreign key (tariff_kind_id)
      references TariffKindsRef (tariff_kind_id)
      on delete restrict on update restrict;

alter table TariffCalculations
   add constraint FK_TARIFFCA_TC_TARIFF_TARIFFSE foreign key (tariff_serv_id)
